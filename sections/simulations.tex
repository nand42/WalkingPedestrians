...
The aim of this paragraph is to give a simplistic example.
Assume a finite grid of cell, a $6\times3$ matrix, where $x$ is horizontal and $y$ is vertical.
Assume that for each position $(x, y)$ is given a vector of \emph{nine} entries, that is exactly the $D2Q9$-model.
Assume a finite number of possible move distributions: three for the first example.
Lets represent the vector in the form of a matrix, referencing to the (Figure \ref{fig:D2Q9_k}).
Lets call them $A, B, C, D$ with following structure:
\begin{equation}
\begin{split}
A=
\left(
